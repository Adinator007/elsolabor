...
class App(val canvas : HTMLCanvasElement, val overlay : HTMLDivElement) {
  ...
  fun registerEventHandlers() {
    document.onkeydown =  { //#{# locally defined function
      ...
      if (keyNames[event.keyCode] == "V"){
        scene.visible = !scene.visible
      }
      if (keyNames[event.keyCode] == "S"){
        scene.solide = !scene.solide
      }
      if (keyNames[event.keyCode] == "G"){
        scene.reset()
      }
    }
    ...
    canvas.onmousedown = { 
      ...
      scene.moveTheTriangle(event.x.toInt(), event.y.toInt(), canvas.width, canvas.height)
